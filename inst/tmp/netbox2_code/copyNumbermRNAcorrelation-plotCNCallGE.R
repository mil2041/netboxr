...
plotCNCallGE<-function(dat,levels,geneSymbol,verbose=TRUE){
  ...
  #NumberOfLevels<-length(levels(dat$conds))
  NumberOfLevels<-5
  ...
  boxplot(dat$geneCounts ~ dat$conds, col=mycolors,outpch=NA,
        na.action=na.exclude,varwidth=TRUE)
  #boxplot(dat$geneCounts ~ dat$conds, col=mycolors,outpch=NA)
  #stripchart(dat$geneCounts ~ dat$conds, vertical=TRUE, data=dat, method="jitter",add=TRUE,pch=16,col=mycolors,cex=0.5)
  ...
  for(i in 1:length(mylevels))
  {
    ...
    points(myjitter, thisvalues, pch=19, col=rgb(0,0,0,.5),cex=1.2) 
    ...
    text(i+levelProportions[i]/2, TopOfWhisker, labels=paste("N=", length(thisvalues), sep=""), cex=.95, font=10, pos=4)
