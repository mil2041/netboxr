...
getMutSig2CVMat<-function(date="last",cancerType="BRCA",workDir,verbose=TRUE){
    ...
    download.file(url=download_link,destfile=file.path(destDir,"MutSig2CV.tar.gz"),method="auto",quiet = FALSE, mode = "w")
    ...
    tmpMat<-read.table(file=file.path(destDir,"sig_genes.txt"),header=TRUE,
