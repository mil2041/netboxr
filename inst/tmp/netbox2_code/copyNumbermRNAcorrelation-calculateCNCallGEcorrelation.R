...
#' @concept netboxr
#' @export
calculateCNGEcorr<-function(log2CNMat,CNCallMat,log2GEMat,ampGeneList,
                            delGeneList,method="MannWhitney",
...
buildcorrelationTest<-function(log2CN,log2GE)
{
    ...
    for(i in 1:probeSize )
    {
        ...
        if( naNumCN > threshold || naNumGE > threshold ){
           ...
           SpearmanTest<-cor.test(t(log2CN[i,]),t(log2GE[i,]),method="kendall",exact=FALSE,na.action=na.exclude)
           ...
           plot(as.matrix(log2CN[i,]),as.matrix(log2GE[i,]),
                main=paste(rownames(log2CN)[i]),xlab="log2(Copy Number value)",ylab="log2(RNA-seq-count-RSEM)",
                pch=16,col="blue",cex=.9) 
           abline(lm(t(log2GE[i,]) ~ t(log2CN[i,]),na.action=na.exclude),col="red",lty=2)

           usr<-par("usr")
           text(usr[1]+0.1,usr[4]-0.2,adj=c(0,1),paste("Pearson Coeff:",round(PearsonTest$estimate,digit=4),",P-Value:",round(PearsonTest$p.value,digit=4),sep=" "))
           text(usr[1]+0.1,usr[4]-0.5,adj=c(0,1),paste("Spearman Coeff:",round(SpearmanTest$estimate,digit=4),",P-Value:",round(SpearmanTest$p.value,digit=4),sep=" "))
           dev.off()
...
buildMannWhitneyTest<-function(CNCall,log2GE,copyNumberType,average="median",rankTestSize=5)
{
        ...
        if( naNumCN > threshold || naNumGE > threshold ){
            ...
            if(copyNumberType %in% "Amp"){
              ...
              if(length(ampCN)>= rankTestSize & length(diploidCN)>= rankTestSize ) {
                ...
                MannWhitneyTest<-wilcox.test(ampCN,diploidCN,
                ...
                #usr<-par("usr")
                #text(usr[1]+0.1,usr[4]-0.4,adj=c(0,1),paste("Two samples Mann-Whitney test",sep=" "))
                #text(usr[1]+0.1,usr[4]-0.8,adj=c(0,1),paste("One-tail pValue:",round(MannWhitneyTest$p.value,digit=5),sep=" "))
                ...
                usr<-par("usr")
                text(usr[1]+0.1,usr[4]-0.4,adj=c(0,1),paste("Two samples Mann-Whitney test",sep=" "))
                text(usr[1]+0.1,usr[4]-0.8,adj=c(0,1),paste("One-tail pValue:",format(MannWhitneyPValue[i],digit=5),sep=" "))
            ...
            if(copyNumberType %in% "Homdel"){
              ...
              if(length(homdelCN)>= rankTestSize) {  
                ...
                MannWhitneyTest<-wilcox.test(homdelCN,diploidCN,
                                             na.action=na.exclude,
                                             exact=FALSE,
                                             alternative="less")
                MannWhitneyPValue[i]<-MannWhitneyTest$p.value
    ...
    data<-data.frame(rownames(CNCall),log2TestMean,log2DiploidMean,log2Fold,MannWhitneyPValue,stringsAsFactors = FALSE)
...
ampTest<-buildMannWhitneyTest(CNCall=CNCallAmp,log2GE=log2GEAmp,copyNumberType="Amp",average,rankTestSize)
...
#ampTest$padj<-p.adjust(ampTest$MannWhitneyPValue,method="BH")
#ampTest$type<-rep("Amp",nrow(ampTest))
...
delTest<-buildMannWhitneyTest(CNCall=CNCallDel,log2GE=log2GEDel,copyNumberType="Homdel",average,rankTestSize)
...
#delTest$type<-rep("Homdel",nrow(delTest))
#delTest<-delTest[order(delTest$padj),]
