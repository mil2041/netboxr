...
unionTwoFrames<-function(mat1,mat2,verbose=TRUE)
{
  ...
  #for(i in 1:length(unionRowName)){
  #  a<-rep(unionRowName[i],length(unionColName))
  #  b<-unionColName
  #  c<-rep(0,length(unionColName))
  #  tmp1<-data.frame(a,b,c)
  #  tmp1$index<-paste(tmp1$a,tmp1$b,sep="@")
