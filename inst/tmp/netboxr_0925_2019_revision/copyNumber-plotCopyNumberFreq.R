...
plotCopyNumberFreq <- function(dataWithPos, threshold, chromSize, verbose = TRUE) {
    ...
    if (nchr == 1) {
        ...
        for (i in 1:max(dataWithPos$index)) {
            # dataWithPos[dataWithPos$index==i, ]$pos <- (dataWithPos[dataWithPos$index==i, ]$startPos -
            # dataWithPos[dataWithPos$index==i,]$startPos[1]) +1 +ticks[i] ticks[i+1] <-
            # max(dataWithPos[dataWithPos$index==i,]$pos) cat(sprintf('index:%s\n',i))
    ...
    xmax <- max(dataWithPos$pos) + (1e+08)
    xmin <- min(dataWithPos$pos) - (1e+08)
    ...
    # xaxt='n',xaxs='i',las=1, yaxt='n',yaxs='i', ,cex.axis=cex.axis,xlab=xlabel,ylab='Frequency (%)')
    ...
    ##### 
    plot(dataWithPos$pos, dataWithPos[, 6] * 100, type = "h", col = "red", xlim = c(xmin, xmax), ylim = c(ymin, 
        ymax), xaxt = "n", xaxs = "i", las = 1, yaxt = "n", yaxs = "i", , cex.axis = cex.axis, xlab = xlabel, 
        ylab = "Frequency (%)")
    ...
    legend("topright", legend = c("Amplification (+2)", "Deletion (-2)"), col = c("red", "blue"), pch = 20, 
        cex = 0.9, xpd = TRUE)
    ...
    axis(side = 2, at = minorTicks, tck = 0.01, labels = NA)
