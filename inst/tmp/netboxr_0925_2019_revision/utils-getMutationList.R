...
# # mutSig2CVthreshold, rareMutationUpperLimit, # rareMutationLowerLimit, rareMutationFreq)
#' 
#' @concept netboxr
#' @export 
#' @importFrom HGNChelper checkGeneSymbols
getMutationList <- function(date, cancerType, selectedSampleId = NA, workDir, mutSig2CVthreshold = 0.1, 
    rareMutationUpperLimit = 0.3, rareMutationLowerLimit = 0.1, rareMutationFreq = 0.02, verbose = TRUE) {
    ...
    if (FALSE) {
        ...
        tmpData <- read.table(fileName, sep = "\t", header = TRUE, stringsAsFactors = FALSE, fill = TRUE, 
            quote = NULL, comment = "")
        
        dd <- mutSig2CVMat[mutSig2CVMat$suggestedSymbol %in% tmpData$Gene.Symbol, ]
        # nn<-dd[order(-dd$freq),] head(nn[nn$freq>=0.02,])
    ...
    if (FALSE) {
        ...
        # filePath<-file.path(PROJHOME,'annotation/kinase_list') fileName<-'kinbase_03102015.txt'
        # fileName<-file.path(filePath,fileName)
        
        fileName <- system.file("extdata", "kinbase_03102015.txt", package = "netboxr")
        
        tmpData3 <- read.table(fileName, sep = "\t", header = TRUE, stringsAsFactors = FALSE, fill = TRUE, 
            quote = NULL, comment = "")
    ...
    mutationResult <- list(mutationList = sigGenesMutation$gene, mutationFreq = sigGenesMutation$freq, 
