...
getRNASeqV2RawCounts <- function(date = "last", cancerType, workDir, verbose = TRUE) {
    ...
    if (!file.exists(file.path(destDir, "RSEM_gene_combined.Rd"))) {
        ...
        for (i in 1:length(plinks)) {
            ...
            download.file(url = download_link, destfile = file.path(destDir, "RSEM_genes_data.tar.gz"), 
            ...
            tmpMat[[i]] <- fread(file.path(destDir, fileName), sep = "\t", header = TRUE, stringsAsFactors = FALSE, 
                data.table = FALSE)
