...
intersectCNGEMat <- function(CNValueMat, CNCallMat, GEMat, geneList = NULL, verbose = TRUE) {
    ...
    mergedData <- list(CNValueMat = CNValueMatSub[order(rownames(CNValueMatSub)), ], CNCallMat = CNCallMatSub[order(rownames(CNCallMatSub)), 
        ], GEMat = GEMatSub[order(rownames(GEMatSub)), ], log2CNValueMat = log2CNValueMatSub[order(rownames(log2CNValueMatSub)), 
        ], log2GEMat = log2GEMatSub[order(rownames(log2GEMatSub)), ], missedGene = missedGene)
