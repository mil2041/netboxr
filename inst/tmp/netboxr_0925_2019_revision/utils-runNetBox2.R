...
runNetBox2 <- function(dataDir, cancerType, mutationList, ampGeneList, delGeneList, epiSilencedList, mutationFreq, 
    ampGeneFreq, delGeneFreq, epiSilencedFreq, pathwayCommonsDb, directed, linkerPValThreshold, communityDetectionMethod, 
    keepIsolatedNodes, verbose = TRUE) {
    ...
    # intersect(epiSilencedList,ampGeneListFiltered) intersect(mutationList,delGeneListFiltered)
    ...
    # keep<-!duplicated(alterationFreq$geneSymbol) alterationFreq<-alterationFreq[keep,]
    ...
    ## check 573 cancer census genes filePath<-file.path(PROJHOME,'annotation/Cosmic_census_list')
    ## fileName<-'cosmic_CGC_0128_2016.txt' fileName<-file.path(filePath,fileName)
    
    fileName <- system.file("extdata", "cosmic_CGC_0128_2016.txt", package = "netboxr")
    tmpData <- read.table(fileName, sep = "\t", header = TRUE, stringsAsFactors = FALSE, fill = TRUE, quote = NULL, 
        comment = "")
    
    ## check 720 epi enzymes filePath<-file.path(PROJHOME,'annotation/epigenetic_gene_list/epifactors')
    ## fileName<-'EpiGenes_main_genes.txt' fileName<-file.path(filePath,fileName)
    
    fileName <- system.file("extdata", "EpiGenes_main_genes.txt", package = "netboxr")
    tmpData2 <- read.table(fileName, sep = "\t", header = TRUE, stringsAsFactors = FALSE, fill = TRUE, 
        quote = NULL, comment = "")
    ...
    if ("BioGrid" %in% pathwayCommonsDb) {
        ...
        # dim(networkBioGrid) names(networkBioGrid)
    ...
    # networkBioGridE3<-networkBioGrid[networkBioGrid$EVIDENCE_NUM==1,] trend<-table(networkBioGridE3$YEAR)
    # plot(trend) networkBioGridE5<-networkBioGridE3[networkBioGridE3$YEAR>=2010,] dim(networkBioGridE5)
    ...
    result <- geneConnector(geneList = geneList, networkGraph = graphReduced, directed = directed, pValueAdj = "BH", 
        pValueCutoff = linkerPValThreshold, communityMethod = communityDetectionMethod, keepIsolatedNodes = keepIsolatedNodes)
    ...
    folderName <- paste("network_numOfLinker", numOfLinker, "communityMethod", communityDetectionMethod, 
        sep = "_")
    ...
    write.table(result$netboxOutput[1:3], file = file.path(filePath, "network.sif"), sep = "\t", quote = FALSE, 
        col.names = FALSE, row.names = FALSE)
    write.table(result$netboxOutput, file = file.path(filePath, "networkAttr.txt"), sep = "\t", quote = FALSE, 
        col.names = TRUE, row.names = FALSE)
    write.table(result$neighborData, file = file.path(filePath, "neighborList.txt"), sep = "\t", quote = FALSE, 
        col.names = TRUE, row.names = FALSE)
    
    if (communityDetectionMethod != "none") {
        fileName <- paste("membership.", communityDetectionMethod, ".txt", sep = "")
        write.table(result$moduleMembership, file = file.path(filePath, "memb.ebc.txt"), sep = "\t", quote = FALSE, 
            col.names = FALSE)
    }
    
    write.table(result$nodeType, file = file.path(filePath, "nodeType.txt"), sep = "\t", quote = FALSE, 
        col.names = FALSE, row.names = FALSE)
    
    
    write.table(alterationFreq, file = file.path(filePath, "alterationFreq.txt"), sep = "\t", quote = FALSE, 
        col.names = FALSE, row.names = FALSE)
    write.table(alterationList, file = file.path(filePath, "alterationAnno.txt"), sep = "\t", quote = FALSE, 
        col.names = FALSE, row.names = FALSE)
    
    
    if (numOfLinker > 0) {
        write.table(linkerDF, file = file.path(filePath, "linkerAnno.txt"), sep = "\t", quote = FALSE, 
            col.names = FALSE, row.names = FALSE)
    }
    
    cat(sprintf("NetBox2 output is located at %s\n", filePath))
    
    netBox2Result <- list(netboxGraph = result$netboxGraph, graphReduced = graphReduced, moduleMembership = result$moduleMembership, 
        sifOutput = result$netboxOutput[1:3], neighborData = result$neighborData)
