...
source('utils/getCancerType.R')
source('utils/getRunDates.R')
source('utils/getGISTIC2Mat.R')
source('utils/getGISTIC2CNValueMat.R')
source('copy_number/plotCopyNumberFreq.R')
source('utils/getRNAseqV2RawCounts_v3.R')
source('CopyNumber_mRNA_correlation/calculate_CNCall_GE_correlation_v7.R')
source('CopyNumber_mRNA_correlation/plotCNCallGE.R')
source('utils/processRawCountsMat_v3.R')
source('utils/processCNValueMat_v3.R')
source('utils/processCNCallMat_v2.R')
source('utils/intersectCNGEmat.R')
source('copy_number/buildCopyNumberFreqMatrix.R')
source('utils/processGECountsMat.R')
source('oncoprint/makeCopyNumberMatrix.R')
...
#filePath<-file.path(PROJHOME,"TCGA_PRAD","input","clinical")
#fileName<-"table_S1A.txt"
...
gExpMatTumorSub<-gExpMatTumor[,which(substr(colnames(gExpMatTumor),1,12) %in% selectedSampleID)]
...
#library("genefilter")
#GE_var_dist<-data.frame(log(rowMeans(rawCountsMat$tumorMat),base=2),rowSds(rawCountsMat$tumorMat),rawCountsMat$geneSymbolTable)
#colnames(GE_var_dist)<-c("mean","variance","geneSymbol","entrezID")
#plot(GE_var_dist$mean,GE_var_dist$variance)

#library("limma")
#voom(rawCountsMat$tumorMat,plot=TRUE)

#GEMat<-rawCountsMat$tumorMat
...
#CNgeneSymbolTable<-gistic2CNCallMat$geneSymbolTable
#GEgeneSymbolTable<-rawCountsMat$geneSymbolTable
...
#corrTestCombined$bonferroni<-corrTestCombined$MannWhitneyPValue*nrow(corrTestCombined)
#corrTestCombined$fdr<-p.adjust(corrTestCombined$MannWhitneyPValue,method="BH")
#corrTestCombined<-corrTestCombined[order(corrTestCombined$copyNumberType,corrTestCombined$bonferroni),]

#corrThreshold<-0.05
#filteredGeneList<-corrTestCombined[corrTestCombined$bonferroni>corrThreshold,]$geneSymbol
#keepedGeneList<-corrTestCombined[corrTestCombined$bonferroni<=corrThreshold,]
#keepedGeneList<-keepedGeneList[order(keepedGeneList$copyNumberType,keepedGeneList$bonferroni),]
