...
results <- sapply(genes, function(gene) {
  ...
  cna <- dat$paad_tcga_linear_CNA
  cna[is.nan(cna) == TRUE] = NA
  cna <- as.factor(as.numeric(cna))
  cna <- as.numeric(levels(cna))[cna]
  
  mut <- dat$paad_tcga_mutations
  mut[is.nan(mut) == TRUE] = NA
  mut <- as.factor(as.character(mut))
