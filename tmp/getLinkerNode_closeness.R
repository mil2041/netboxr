...
vertex_closeness_list<-mclapply(1:iterations, function(m)
{  
  ...
  # isolated nodes will make distance become "Inf", it makes error in calculate rowSums
  vertex_closeness_vector2<-graphTemp3 %>% 
                          indirect_relations(type="dist_sp") %>% 
                          aggregate_positions(type="invsum")
  
  return(vertex_closeness_vector2)
  
},mc.cores=useCores)

s1<-unlist(vertex_closeness_list)
...
pRaw<-{}
  ...
  #vertex_closeness_vector<-closeness(graphTemp2,vids=neighborListFrame$name,mode="all",normalized = FALSE)
  
  vertex_closeness_vector<-graphTemp2 %>% 
    indirect_relations(type="dist_sp") %>% 
    aggregate_positions(type="invsum")

for(i in 1:length(neighborListFrame$name)){
  
  vertex_closeness_vector0<-vertex_closeness_vector[names(vertex_closeness_vector)==neighborListFrame$name[i]]
  
  s2<-s1[names(s1) %in% neighborListFrame$name[i]]
  
  pRaw[i]<-(sum(s2>=vertex_closeness_vector0)+1)/(iterations+1)
