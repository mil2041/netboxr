...
for(selectedName in unique(originalNeighborListFrame$name)){
  ...
  observedScore<-originalNeighborListFrame[originalNeighborListFrame$name %in% selectedName,]$oddsRatio
  simulatedScores<-cc[cc$name %in% selectedName,]$oddsRatio
  pValueSample[selectedName]<-(sum(observedScore>=simulatedScores)+1) / (length(cc[cc$name %in% selectedName,]$oddsRatio) +1)
