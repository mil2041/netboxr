...
vertex_harmonic_list<-mclapply(1:iterations, function(m)
{  
  ...
  vertex_harmonic_vector2<-graphTemp3 %>% 
                          indirect_relations(type="dist_sp",FUN=dist_inv) %>% 
                          aggregate_positions(type="sum")
  
  return(vertex_harmonic_vector2)
  
},mc.cores=useCores)

s1<-unlist(vertex_harmonic_list)
...
pRaw<-{}
  ...
  vertex_harmonic_vector<-graphTemp2 %>% 
    indirect_relations(type="dist_sp",FUN=dist_inv) %>% 
    aggregate_positions(type="sum")

for(i in 1:length(neighborListFrame$name)){
  
  vertex_harmonic_vector0<-vertex_harmonic_vector[names(vertex_harmonic_vector)==neighborListFrame$name[i]]
  
  s2<-s1[names(s1) %in% neighborListFrame$name[i]]
  
  pRaw[i]<-(sum(s2>=vertex_harmonic_vector0)+1)/(iterations+1)
