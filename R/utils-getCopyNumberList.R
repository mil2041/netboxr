...
getCopyNumberList <- function(date, cancerType, selectedSampleId = NA, workDir, copyNumberFreqThreshold = 0.05, 
    corrAverage = "median", minRankTestSize = 4, indepThreshold = 3, corrThreshold = 0.05, logFCThreshold = 0.5, 
    verbose = TRUE) {
    
    # runDates<-getRunDates() date<-runDates[1] date<-'2014_10_17'
    ...
    CNCallMat <- gistic2CNCallMatTumor[, which(substr(colnames(gistic2CNCallMatTumor), 1, 12) %in% substr(selectedSampleId, 
        1, 12))]
    ...
    # fileName<-'hg19_annotation_by_gene_plot.txt' fileName<-file.path(filePath,fileName)
    ...
    gExpMatTumorSub <- gExpMatTumor[, which(substr(colnames(gExpMatTumor), 1, 12) %in% substr(selectedSampleId, 
        1, 12))]
    ...
    if (!file.exists(fileName)) {
        ...
        corrTable <- calculateCNGEcorr(log2CNMat, CNCallMat, log2GEMat, ampGeneList, delGeneList, method = "MannWhitney", 
            average = corrAverage, rankTestSize = minRankTestSize, workDir = filePath)
        names(corrTable)
    ...
    # independent filtering remove diploid median less than log(2,base=2) indepThreshold<-3
    ...
    # bb<-merge(corrTestIndep,dataWithPos,by='geneSymbol') bb<-bb[order(bb$copyNumberType,bb$fdr),]
    # bb1<-bb[bb$copyNumberType %in% 'Amp',] bb2<-bb[bb$copyNumberType %in% 'Homdel',]
    ...
    plot(delGroup$log2Fold, -log(delGroup$bonferroni, base = 10), pch = 20, col = "blue", cex = 1.5, xlab = "log2FoldChange [ extreme / diploid ]", 
        ylab = "-log10 ( pValue-adjusted )", xlim = c(min(delGroup$log2Fold, ampGroup$log2Fold), max(delGroup$log2Fold, 
            ampGroup$log2Fold)), ylim = c(0, max(-log(delGroup$bonferroni, base = 10), -log(ampGroup$bonferroni, 
            base = 10))))
    ...
    legend("topright", pch = 20, col = c("red", "blue", "green", "grey"), legend = c("Amp", "Homdel", pValueLegend, 
        logFCLegend))
    mtext(paste(totalCNVnum - removedCNVnum, " passed / ", totalCNVnum, " total, rate: ", signif(1 - filteredRatio, 
        digit = 2), sep = ""))
    dev.off()
    ...
    copyNumberList <- list(ampGeneList = ampGeneListFiltered, ampGeneFreq = ampGeneFreq, delGeneList = delGeneListFiltered, 
        delGeneFreq = delGeneFreq, copyNumberMat = copyNumberMat)
