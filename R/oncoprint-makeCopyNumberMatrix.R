...
makeCopyNumberMatrix<-function(CNCallMat,selectedSampleId,geneList,verbose=TRUE){
  ...
  tmpCNCallMat<-CNCallMat[,which(substr(colnames(CNCallMat),1,12) %in% substr(selectedSampleId,1,12))]
  overlapSampleID<-intersect(unique(substr(colnames(tmpCNCallMat),1,12)),unique(substr(selectedSampleId,1,12)))
