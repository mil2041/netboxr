...
processCNCallMat<-function(CNCallMat,verbose=TRUE){
  ...
  #entrezID<-CNCallMat[,2]
  #geneSymbol<-CNCallMat[,1]
  #CNCallgeneSymbolTable<-data.frame(geneSymbol,entrezID)
  #CNCallMat<-CNCallMat[,4:ncol(CNCallMat)]
  #rownames(CNCallMat)<-entrezID
  #sampleIDs<-gsub("-",".",colnames(CNCallMat))
  ...
  if( length(normalSamples)>0 ){
    #CNCallSamplesNormal<-normalSamples
    #CNCallMatNormal<-CNCallMat[,gsub("\\.","-",normalSamples)]
