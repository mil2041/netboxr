...
processRawCountsMat <- function(rawCountsMat, normalization = "TMM") {
    ...
    # file_fn<-'RSEM_genes_sample_id.txt' id_df<-read.table(file_fn,sep='\t',header=FALSE)
    ...
    # if( length(normalSamples)>0 ){ GESamplesNormal<-gsub('-','.',normalSamples) }
    ...
    # remove character lable row rawCountsMat<-rawCountsMat[-1,keep] convert back to numeric column type
