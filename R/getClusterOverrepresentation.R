...
getClusterOverrepresentation <- function(clusters, gmtFile, totalEntriesNum = NULL, verbose = FALSE) {
    ...
    for (listEntry in 1:length(gmtList)) {
        gmtList[[listEntry]] <- gmtList[[listEntry]][which(gmtList[[listEntry]] != "" & gmtList[[listEntry]] != 
            "-")]
    }
    ...
    # totalEntriesNum is needed for phyper so it must be calculated prior to the iteration.
    ...
    for (clusterIdx in 1:length(clusters)) {
        for (gmtEntryIdx in 1:length(gmtList)) {
            entriesInGmt <- intersect(gmtList[[gmtEntryIdx]], clusters[[clusterIdx]])
            ...
            pval <- phyper(entriesInGmtNum - 1, gmtEntryNum, totalEntriesNum - gmtEntryNum, clusterEntryNum, 
                lower.tail = FALSE)
            ...
            if (verbose) {
                cat("GN: ", entriesInGmtNum, " PN: ", gmtEntryNum, " TG: ", totalEntriesNum, " CG: ", clusterEntryNum, 
                  " P: ", pval, " CI: ", clusterIdx, " PI: ", gmtEntryIdx, " P: ", gmtEntryName, " G: ", 
                  entriesInGmt, "\n")
            }
        }
    }
    ...
    results <- data.frame(entriesInGmtNum = entriesInGmtNumVec, gmtEntryNum = gmtEntryNumVec, totalClusterEntriesNum = totalClusterEntriesNumVec, 
        clusterEntryNum = clusterEntryNumVec, clusterIdx = clusterIdxVec, gmtEntryIdx = gmtEntryIdxVec, 
        gmtEntryName = gmtEntryNameVec, entriesInGmt = entriesInGmtVec, pval = pvalVec, pvalAdjust = pvalAdjust)
