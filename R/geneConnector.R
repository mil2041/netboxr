...
geneConnector <- function(geneList, networkGraph, directed = FALSE, pValueAdj = "BH", pValueCutoff = 0.05, 
    ...
    neighborListFrame <- data.frame(neighborList$reducedGraphId, neighborList$name, neighborList$localDegree, 
        neighborList$globalDegree, neighborList$pValueRaw, neighborList$oddsRatio, stringsAsFactors = FALSE)
    ...
    graphTemp <- set_vertex_attr(graphTemp, name = "nodeType", index = as.character(linkerListFrame$name), 
        value = "linker")
